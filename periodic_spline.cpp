...
namespace geom
{
                                   ...
                                   int degree)
    {
        ...
        // within the range, t_\mu, t_{\mu+1} the spline evaluation
        // depends on  t_{\mu-d+1}.....t_\mu,t_{\mu+1},....t_{\mu+d} (check the R_d matrix)
        //  ./rmatrix.png
        // at \mu = 0, we need to ensure that t_{\mu-d+1} and accessible, so
        // d-1 extra knots are prefixed here at \mu = ks.size()-2, we need to
        // ensure that t_{\mu+d} is accessible, so d-1 extra knots are
        ...
        startDiffs[0] = ks.back();  //change to ks_{n-1} in preparation for the partial sums below
        endDiffs[0] = ks.front();   //change the start of backwardly oriented end diffs to ks_0
        ...
        t.insert(t.end(),ks.begin() + 1, ks.end()); // already copied ks.front() so the +1
        t.insert(t.end(),back_plus.begin() + 1, back_plus.end()); //already copied ks.back()
        return std::forward_as_tuple(std::move(cpts),std::move(t),std::move(degree));
    }

}
#include "point.hpp"
template struct geom::periodic_bspline<geom::pt_t<2>>;
