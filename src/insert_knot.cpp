             ...
             const std::vector<double>& new_knots)
{
    ...
    typedef typename SplineType::cpts_t cpts_t;
    knots_t taus;
             ...
             geom::regular_tag )
{
    ...
    return SplineType(std::move(newcpts),std::move(newts),p);
}
             ...
             geom::periodic_tag)
{
    ...
    if( nu < size_t(2*p) )
    {
        ...
        auto & t0  = crv0.knots();
        std::adjacent_difference(t0.cbegin() + p, t0.cbegin() + 2*p  + 1,
                                 startDiffs.begin());
        startDiffs[0] = t0.cend()[-p-1];
        std::vector<double> ks(p+1);
        std::partial_sum(startDiffs.cbegin(), startDiffs.cend(), ks.begin());
        return rebase_at_end(crv0, ks.cbegin());
    }else if( nu > t.size() - 2 * p - 1)
    {
        std::vector<double> endDiffs(p+1);
        auto &t0 = crv0.knots();
        std::adjacent_difference(t0.cend() - 2*p - 1,  t.cend() - p,
                                 endDiffs.begin());
        endDiffs[0] = t0[p];
        std::vector<double> ks(p+1);
        std::partial_sum(endDiffs.cbegin(),endDiffs.cend(), ks.begin());
...
bspline_ops::insert_knot(const SplineType& crv,
                         double u)
{
   return insert_knot_impl(crv,u,spline_traits<SplineType>::ptag());
}
...
eval:(instantiate-templates "insert_knot" "bspline_ops" (list ) methods spltypes )
End:
