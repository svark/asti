...
TEST_CASE("approxtestbasic", "[bspline][approximation]") {
  ...
  INFO("cube approx at -1,0.1,0,1:" << spl0.eval(-1) << "," << spl0.eval(0.1)
                                    << ',' << spl0.eval(0) << ','
                                    << spl0.eval(1) << "\n");
  ...
  std::merge(bs.knots().cbegin(), bs.knots().cend(), arr, arr + 2,
             newknots.begin());
  newknots.insert(newknots.begin(), 0.0);
  newknots.insert(newknots.end(), 1.0);
  auto v_vdash = [&p, &bs](double u) -> double {
    auto v = (bs.eval(u) - p);
    auto vdash = bs.eval_derivative(1, u);
    return dot(v, vdash);
  };

  auto spl2 = geom::ops::cubic_approx1d(std::function<double(double)>(v_vdash),
                                        newknots);
  auto pt_ = spl2.eval(-1);
  INFO("\n dist.dist'  at 0.1,0.2,1:" << v_vdash(0.25) << ',' << v_vdash(0.2)
                                      << ',' << v_vdash(.5) << "\n");
  INFO("\n dist.dist' approx at 0.1,0.2,1:" << spl2.eval(0.25) << ','
                                            << spl2.eval(0.2) << ','
                                            << spl2.eval(.5) << "\n");
...
TEST_CASE("cubic hermite interp", "[bspline][interpolation][approximation]") {
  ...
  SECTION("not a knot") {
    ...
    INFO("v:" << bs.eval(0.2) << "," << bs.eval(0.3806) << "," << bs.eval(0.4)
              << "," << bs.eval(0.5) << "," << bs.eval(0.6) << ","
              << bs.eval(0.9) << "," << bs.eval(1.1) << "\n");
  ...
  SECTION("vanishing double derivatives") {
    auto bs = geom::piecewise_cubic_hermite_interp(ps.begin(), ps.end(), opts,
                                                   std::vector<double>(6, 0));
    INFO("v:" << bs.eval(0.2) << "," << bs.eval(0.4) << "," << bs.eval(0.5)
              << "," << bs.eval(0.6) << "," << bs.eval(0.9) << ","
              << bs.eval(1.1) << "\n");
  ...
  SECTION("periodic") {
    ...
    INFO("v:" << bs.eval(0) << "," << bs.eval(0.2) << "," << bs.eval(0.3806)
              << "," << bs.eval(0.4) << "," << bs.eval(0.5) << ","
              << bs.eval(0.6) << "," << bs.eval(0.9) << "," << bs.eval(1.1)
              << "\n");
    ...
    // INFO ("is periodic" << std::boolalpha << ":" <<
    // geom::ops::is_periodic(bs.spline()) << "\n\n");
    for (auto k : bs.knots()) {
      INFO("at " << k << ": " << bs.eval(k) << "\n");
    }
    INFO("------------\n");
